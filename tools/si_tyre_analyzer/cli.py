...
def _cmd_info(args):
    ...
    print(f"wheel={s.wheel} session_id={s.session_id} grid={s.cols}x{s.rows} "
...
def _cmd_dashboard(args):
    ...
    sid = max({s.session_id for s in sessions},
...
def _cmd_fetch(args):
    if args.all:
        ...
        for s in fetch.list_sessions(args.host):
            print(f"{s['wheel']:>2}  {s['name']:<24} {s['records']} recs "
                  f"{s['size']} B")
...
def main(argv=None):
    ...
    sp = sub.add_parser("info"); sp.add_argument("file"); sp.set_defaults(fn=_cmd_info)

    sp = sub.add_parser("heatmap"); sp.add_argument("file")
    sp.add_argument("--save"); sp.set_defaults(fn=_cmd_heatmap)

    sp = sub.add_parser("timeseries"); sp.add_argument("file")
    sp.add_argument("--save"); sp.set_defaults(fn=_cmd_timeseries)

    sp = sub.add_parser("dashboard"); sp.add_argument("dir")
